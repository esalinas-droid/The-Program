...
export default function BuildPlanScreen() {
  ...
  const runExtraction = useCallback(async () => {
    ...
    try {
      ...
      // Only set isReactivation=true if:
      //  1. user has previously imported a program (has_imported_program === true)
      //  2. an active plan already exists (active?.startDate)
      //  3. SAME document is being activated again (active?.sourceDocumentId === id)
      //     Prevents "Re-activate plan" on new/different documents.
      try {
        const prof = await getProfile();
        const hasImportedBefore = (prof as any)?.has_imported_program === true;
        const { active } = await programsApi.list();
        if (active?.startDate && hasImportedBefore && (active as any)?.sourceDocumentId === id) {
...
// ── Styles ─────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// StartFromPicker is now imported from src/components/StartFromPicker.tsx
